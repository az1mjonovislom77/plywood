...
from utils.service.comprehensive_stats import (
    ...
    def calculate(
        ...
        out_revenue_qs = (
            OrderItem.objects.filter(
                MaterialReportJsonService._accepted_order_range_filter(
                    start_dt,
                    end_dt,
                )
            )
            ...
            .annotate(
                out_revenue_in_dollar=Coalesce(
                    Sum(
                        ...
                        * Case(
                            When(
                                new_price_in_dollar__isnull=False,
                                then=F("new_price_in_dollar"),
                            ),
        ...
        out_revenue_map = {
            r["product_id"]: Decimal(
                str(
                    r.get(
                        "out_revenue_in_dollar"
                    ) or 0
                )
            )
        ...
        for category in categories:
            for product in category.products.all():
                ...
                revenue = out_revenue_map.get(
                    pid,
                    Decimal("0"),
                )

                cogs = (
                    period_cogs_map_in_dollar.get(
                        pid,
                        Decimal("0"),
                    )
                )
        ...
        kromka = Category.objects.filter(
            name__iexact="KROMKA"
        ).first()
        ...
        return {
            "all_profit_som": float(
                all_profit_som
            ),
            "all_profit_dollar": float(
                all_profit_dollar
            ),
        }